// Netlify serverless function for countdown PNG
...
export const handler = async (event, context) => {
  ...
  try {
    await countdownHandler(req, res, false); // false for PNG mode
    
    return {
      statusCode: res.statusCode || 200,
      headers: {
        'Content-Type': res.headers['Content-Type'] || 'image/png',
        'Cache-Control': res.headers['Cache-Control'] || 'public, max-age=60',
    ...
    console.error('Countdown function error:', error);
    return {
      statusCode: 500,
      headers: {
